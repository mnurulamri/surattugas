...
<div>
	...
	<ul class="col-xs-12 col-md-12 pull-left text-info" style="font-size:12px;">
		<li>Silahkan tekan tombol <font class="label label-primary">approval</font> untuk menyetujui permohonan surat tugas yang sudah diajukan Manajer SDM</li>
		<li>Permohonan yang sudah disetujui oleh Wakil Dekan akan merubah status menjadi "Disetujui"</i>
		<li>Silahkan tekan tombol <font class="label label-warning">view</font> untuk melihat detail data permohonan</li>
		<li>Wakil Dekan dapat memantau aktifitas pergerakan penerbitan surat tugas melalui status persetujuan yang meliputi:
			<ul>
		...
		<div class="modal-content">
			...
			<div class="modal-body" style="overflow:auto">
				<div id="view-data-perbaikan">
					<textarea id="catatan_perbaikan_wadek" name="catatan_perbaikan_wadek" rows="5" cols="80"></textarea>
				</div>
			</div>
			<div class="modal-footer">
...
<div class="modal fade" id="tolakModal" tabindex="-1" role="dialog" aria-labelledby="tolakModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body" style="overflow:auto">
				<div id="view-data-perbaikan">
					<textarea id="catatan_tolak_wadek" name="catatan_tolak_wadek" rows="5" cols="80"></textarea>
				</div>
			</div>
			<div class="modal-footer">
...
<div class="modal fade" id="uploadArsipModal" tabindex="-1" role="dialog" aria-labelledby="uploadArsipModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
        ...
        <form id="upload_form" enctype="multipart/form-data" method="post">
          ...
          <input type="file" name="file_upload" id="file_upload" onchange="uploadFileConfirm()"><br>
...
<?php //include "surtu_list_wadek_script.php"; ?>
<script src="../ckeditor/ckeditor.js"></script>
<script>
$(document).ready(function(){
	CKEDITOR.replace('catatan_perbaikan_wadek')
	CKEDITOR.replace('catatan_tolak_wadek')
	
	fetchList()
	
	function fetchList(){
		$.post("views/surtu_list_wadek_ajax.php", function(data) {
	...
	$(document).on("click", "#setujui", function(){
		...
		if (r == true) {
			$.ajax({
				...
				type: "POST",
				data: {kd_surtu:kd_surtu, crud:10},
				success: function(data)
				{
					$("#approvalModal").modal("hide")
					fetchList()
					//alert(data)
					//$("#test-script").html(data)
				}
			})
		}
	})
	...
	$(document).on("click", "#simpan-perbaikan", function(){
		var kd_surtu = $("#kode-surtu").val()
		var catatan = CKEDITOR.instances.catatan_perbaikan_wadek.getData();
		...
		$.ajax({
			url: "views/surtu_crud.php",
			type: "POST",
			data: {kd_surtu:kd_surtu, catatan_perbaikan_wadek:catatan, crud:12},
			success: function(data)
			{
				fetchList()
			},
	...
	$(document).on("click", "#simpan-tolak", function(){
		var kd_surtu = $("#kode-surtu").val()
		var catatan = CKEDITOR.instances.catatan_tolak_wadek.getData();
		//alert(kd_surtu+ ' ' +catatan)
		...
		$.ajax({
			url: "views/surtu_crud.php",
			type: "POST",
			data: {kd_surtu:kd_surtu, catatan_tolak_wadek:catatan, crud:11},
			success: function(data)
			{
				fetchList()
			},
	...
	$('#viewDokumenModal').on('hidden.bs.modal', function() {
		//$(".modal-body #dokumen-view").html('processing...')
	})
