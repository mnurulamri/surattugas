	...
	<table class="table frm-list" style="font-size:12px">
		<thead>
			<tr>
				...
				<th colspan="1" style="text-align:center">Final</th>
			</tr>
		</thead>
		<tbody>';
		$no=1;
		foreach ($result_kegiatan as $row){
			...
			//icon draft surat
			if(!empty($draft_st[$row['kd_surtu']])){
				...
				*/
				/*$view_draft = '<a onclick="window.open('.$url.', \'_blank\', \'location=yes,height=570,width=520,scrollbars=yes,status=yes\');">
				  Share Page
				</a>';*/
				
				$view_draft = '<div class="view_file_draft fa fa-file-word-o"  style="color:blue; font-size:18px; cursor:pointer" data-id="'.$row['kd_surtu'].'"></div>';
			...
			$upload_draft = '<div class="upload-draft fa fa fa-arrow-circle-o-up" data-surtu="'.$kd_surtu.'" data-tt="tooltip" title="upload draft surat" style="color:#00a8a8; font-size:20px; cursor:pointer"></div>';
			$upload_arsip = '<div class="upload-arsip fa fa-arrow-circle-o-up" data-surtu="'.$kd_surtu.'" data-tt="tooltip" title="upload final surat" style="color:#008080; font-size:20px;"></div>';
			...
			$html.= '
				<tr id="'.$kd_surtu.'">
					...
					<td>'.$no_surat.'</td>
					<td data-tgl_krgiatan="'.$tgl_kegiatan.'">'.$tgl_kegiatan.'</td>
