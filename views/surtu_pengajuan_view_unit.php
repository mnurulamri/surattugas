...
# fetch data pemohon
/*$sql = "SELECT * FROM surtu_user WHERE username IN (
		SELECT user_pemohon
		FROM surtu_kegiatan 
		WHERE kd_surtu = '$kd_surtu')";*/
$sql = "SELECT * FROM master_employee WHERE username IN (
		SELECT user_pemohon
		FROM surtu_kegiatan 
		WHERE kd_surtu = '$kd_surtu')";
$pdo->query($sql);
$result_pemohon = $pdo->resultset();
foreach ($result_pemohon as $row){
	$nip_pemohon = $row['nip'];
	$nama_pemohon = $row['nama'];
	$telp_pemohon = $row['telp'];
...
foreach ($result_kegiatan as $row){
	...
	$username_pemohon = $row['user_pemohon'];
}
...
#echo $username.' '.$username_pemohon;
#cetak data
/*
if ($username != $username_pemohon){
	$data_pemohon = '
	<tr>
		<th colspan="2" class="head-label">Data Pemohon</th>
	</tr>
	<tr>
		<td>Nama</td><td>: '.$nama_pemohon.'</td>
	</tr>
	<tr>
		<td>NIP/NUP</td><td>: '.$nip_pemohon.'</td>
	</tr>
	<tr>
		<td>PAF/Dept/Prodi</td><td>: '.$array_unit_kerja[$kodebidang].'</td>
	</tr>
	<tr>
		<td>Telp</td><td>: '.$telp_pemohon.'</td>
	</tr>
	<tr>
		<td>e-Mail</td><td>: '.$email_pemohon.'</td>
	</tr>
	<tr>
...
echo '
	...
	<tr>
		<td>NIP/NUP</td><td id="_nip_pejabat">: '.$nip_pejabat.'</td>
	</tr>
	<tr>
...
//eksekusi pengajuan
	$("#ajukan").click(function(){
		...
		var nip_pejabat = $("#_nip_pejabat").text()
		var crud = 5
		
		//eksekusi data yang diajukan
		var r = confirm("Anda akan menyetujui permohonan pembuatan surat tugas " + nama_kegiatan + "?");
		if (r == true) {
			$.ajax({
				url: "views/surtu_crud.php",
				type: "POST",
				data: {kd_surtu:kd_surtu, nip_pejabat:nip_pejabat, crud:crud},
	...
	// perbaikan pengajuan
	//-----------------------------------------------
	CKEDITOR.replace('catatan_perbaikan_unit')
	...
	$(document).on("click", "#simpan-perbaikan", function(){
		...
		$("#pesan-perbaikan").html('<b class="fa fa-spinner"></b>')
		
		var kd_surtu = "<?php echo $kd_surtu; ?>"
		var catatan = CKEDITOR.instances.catatan_perbaikan_unit.getData();
		
		//simpan status perbaikan dan keterangannya
		$.ajax({
			url: "views/surtu_crud_unit.php",
			type: "POST",
			data: {kd_surtu:kd_surtu, catatan_perbaikan_unit:catatan, crud:2},
			success: function(data)
			{
				$("#pesan-perbaikan").html('<b>data sudah disimpan</b>')
				//$("#pesan-perbaikan").html(data)
				setTimeout(function() { $('#pesan-perbaikan').html(''); }, 500);
				fetchList()
			}
		})
	})
	...
	// tolak pengajuan
	//-----------------------------------------------
	CKEDITOR.replace('catatan_tolak_unit')
	...
	$(document).on("click", "#simpan-tolak", function(){
		$("#pesan-tolak").html('<b class="fa fa-spinner"></b>')
		
		var kd_surtu = "<?php echo $kd_surtu; ?>"
		var catatan = CKEDITOR.instances.catatan_tolak_unit.getData();
		//alert(kd_surtu+ ' ' +catatan)
		
		//simpan status penolakan dan keterangannya
		$.ajax({
			url: "views/surtu_crud_unit.php",
			type: "POST",
			data: {kd_surtu:kd_surtu, catatan_tolak_unit:catatan, crud:3},
			success: function(data)
			{
				$("#pesan-tolak").html('<b>data sudah disimpan</b>')
				//$("#pesan-tolak").html(data)
				setTimeout(function() { $('#pesan-tolak').html(''); }, 500);
				fetchList()
			}
		})
	})
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="perbaikanModalLabel">Perbaikan Pengajuan Surat Tugas</h4>
			</div>
...
<div class="modal fade" id="tolakModal" tabindex="-1" role="dialog" aria-labelledby="tolakModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="tolakModalLabel">Penolakan Pengajuan Surat Tugas</h4>
			</div>
			<div class="modal-body" style="overflow:auto">
				<div id="view-data-perbaikan">
