...
function searchFilter(page_num) {
    ...
    $.ajax({
        type: 'POST',
        url: 'views/surtu_list_operator_pagination_ajax.php',
        data:'page='+page_num+'&perihal='+perihal+'&unit_kerja='+unit_kerja+'&status='+status,
        beforeSend: function () {
...
<input type="hidden" id="page-num" value="">
	<fieldset>
		...
		<label>Perihal:</label>
		<input type="text" id="perihal" placeholder="search..." onkeyup="searchFilter()"/>
		<input type="hidden" id="unit_kerja" placeholder="unit_kerja" onkeyup="searchFilter()"/>
		<label>Status:</label>
		<select id="status" onchange="searchFilter()">
			<option value="(1,3,4,5,6,7,8)">Semua Status</option>
