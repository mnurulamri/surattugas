...
if($_SESSION['username']=='budhidharma'){
	$kode_bidang = "'006', '012'";
} else if($_SESSION['username']=='yandryk'){
	$kode_bidang = "'008', '022'";
} else {
...
$sql = "SELECT a.* , NamaBidang as unit_kerja, c.nama_bergelar as nama_pemohon
		FROM surtu_kegiatan a
		JOIN TblBidangDetail b ON a.KodeBidang = b.KodeBidang AND a.KodeBidang IN 
		(
			$kode_bidang
		...
		foreach ($result_kegiatan as $row){
			...
			*/
			...
			$html.= '
				<tr id="'.$kd_surtu.'">
					<td>'.$no.'</td>
					<td class="text-left">'.$nama_pemohon.'</td>
					<td class="text-left">'.$nama_kegiatan.'</td>
					<td class="text-center" data-tgl_kegiatan="'.$tgl_kegiatan.'">'.$tgl_kegiatan.'</td>
					<td class="text-center" data-sumber-dana="'.$row['sumber_dana'].'">'.$sumber_dana.'</td>
